...
class SACExpectile(BaseAgent):
    """
    SAC Expectile implements the Soft Actor-Critic algorithm for continuous action spaces using expectile
    """
    def __init__(
        self,
        gamma,
        ...
        # Expectile-specific additions
        self.expectile = expectile
        self._device = torch.device("cuda" if cuda and torch.cuda.is_available() else "cpu")
        self._value = VMLP(obs_space.shape[0], critic_hidden_dim, init, activation).to(self._device)
        self._value_optim = Adam(self._value.parameters(), lr=critic_lr, betas=betas)
    ...
    def _update_value(self, state_batch, action_batch):
        with torch.no_grad():
            q1, q2 = self._critic(state_batch, action_batch)
            q_min = torch.min(q1, q2)

        v = self._value(state_batch)
        diff = q_min - v
    ...
    def update(self, state, action, reward, next_state, done_mask):
        ...
        self._update_value(state_batch, action_batch)
    ...
    def _update_double_critic(self, state_batch, action_batch, reward_batch,
        ...
        with torch.no_grad():
            ...
            with torch.no_grad():
                v_next = self._value(next_state_batch)
            q_target = reward_batch + mask_batch * self._gamma * v_next
