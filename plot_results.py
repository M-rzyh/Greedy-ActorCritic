...
def load_results_from_folder(results_folder="./results"):
    """Acrobot-v1_SACresults
    Load all pickle files from results folder, grouping by model type.
    Handles base SAC and expectile sweeps with clear model names.
    
    Folder structure assumed:
      - results/no_expectile_sweep/  -> Base SAC
      - results/expectile_sweep/expectile_0.7/  -> SAC expectile 0.7
      - results/expectile_sweep/expectile_0.8/  -> SAC expectile 0.8
      - results/expectile_sweep/expectile_0.9/  -> SAC expectile 0.9
    """
    models_data = defaultdict(list)

    # Base SAC
    base_sac_folder = os.path.join(results_folder, "no_expectile_sweep/Acrobot-v1_SACresults")
    if os.path.exists(base_sac_folder):
        pkl_files = glob(os.path.join(base_sac_folder, "*.pkl"))
        model_name = "no_expectile_SAC n=25"
    ...
    # Expectile SAC folders
    expectile_root = os.path.join(results_folder, "expectile_sweep")
    if os.path.exists(expectile_root):
        for expectile_folder in sorted(os.listdir(expectile_root)):
            folder_path = os.path.join(expectile_root, expectile_folder)
            if os.path.isdir(folder_path):
                pkl_files = glob(os.path.join(folder_path, "*.pkl"))
                model_name = f"expectile_SAC_{expectile_folder.split('_')[-1]} n=25"
                for pkl_file in pkl_files:
                    try:
                        with open(pkl_file, 'rb') as f:
                            data = pickle.load(f)
    ...
    print(f"\nLoaded models: {list(models_data.keys())}")
    return models_data
